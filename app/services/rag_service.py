...
class RAGService:
    ...
    def answer(self, question: str) -> tuple[str, list[dict]]:
        ...
        if self._settings.rag_enabled and self._settings.rag_mode != 'never':
            ...
            sources = self._qdrant.search(
                query_vector, limit=self._settings.top_k)
            filtered = [source for source in sources if source['score']
            ...
            if self._settings.rag_mode == 'auto':
                ...
                if filtered:
                    ...
                    if top_score < self._settings.max_score_strict:
                        use_rag = False
            if use_rag and filtered:
                max_sources = min(self._settings.top_k,
                                  self._settings.max_sources)
    ...
    def _chunk_python(self, text: str) -> list[str]:
        ...
        for line in lines:
            ...
            is_def = is_top_level and (stripped.startswith(
                'def ') or stripped.startswith('class '))
