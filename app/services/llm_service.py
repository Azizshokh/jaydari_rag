...
class LocalLLM:
    def __init__(self, settings: Settings) -> None:
        ...
        device = 0 if settings.llm_device.lower(
        ) != 'cpu' and torch.cuda.is_available() else -1
    ...
    def generate(self, prompt: str) -> str:
        ...
        outputs = generator(
            prompt,
            max_new_tokens=self._settings.llm_max_new_tokens,  # length
            temperature=self._settings.llm_temperature,
            do_sample=self._settings.llm_temperature > 0,
            # sampling enabled | creative response
            pad_token_id=generator.tokenizer.eos_token_id,
        )
        text = outputs[0]['generated_text']  # prompt + generated text
        return text[len(prompt):].strip()  # generated text
